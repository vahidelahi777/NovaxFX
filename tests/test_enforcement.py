...
def test_best_of_200_random_rejected_end_to_end():
    ...
    dec = evaluate_gate(
        campaign_id="camp", strategy="asian", run_id=best_run, artifacts=arts, registry=reg
    )
...
def test_gate_has_no_boolean_override_params():
    ...
    assert not any(
        k in sig.parameters
        for k in ("survives_cost_stress", "beats_randomized_entry", "drawdown_pct")
    )
...
def test_gate_rejects_missing_artifacts():
    ...
    with runner.evaluate(
        ...
        run_id = ev.run_id
    dec = evaluate_gate(campaign_id="c", strategy="s", run_id=run_id, artifacts=arts, registry=reg)
    assert dec.passed is False
...
def test_gate_rejects_provenance_mismatch():
    ...
    arts.register(
        ...
        provenance={**base, "data_hash": "DIFFERENT"},
        payload={"pnls": [1, -1]},
    )
...
def test_legacy_caller_boolean_gate_is_removed():
    ...
    with pytest.raises(RuntimeError, match="removed in Phase 0.7"):
        evaluate_go_no_go(
            object(),
            instruments_passing=4,
            walk_forward_window_pass_rate=1.0,
            lockbox_expectancy_after_costs=1.0,
            survives_cost_stress=True,
            max_pnl_share_per_slice=0.1,
            deflated_sharpe=object(),
            mc_p95_drawdown_pct=0.1,
            drawdown_pct=0.1,
            beats_randomized_entry=True,
            survives_one_bar_delay=True,
        )
